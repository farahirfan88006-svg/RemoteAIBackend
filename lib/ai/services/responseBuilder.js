...
/**
 * Maps an internal error (thrown by a provider, network layer, cache, etc.)
 * to a short, non-technical message safe to send to clients. The original
 * error (with provider name, HTTP status, stack, etc.) is expected to have
 * already been logged server-side via logger.error by the caller — this
 * function never re-logs, it only decides what the *client* is allowed to see.
 *
 * @param {Error|string|null|undefined} err
 * @returns {string}
 */
export function toFriendlyErrorMessage(err) {
  const raw = typeof err === 'string' ? err : err && err.message ? err.message : '';
  const lower = raw.toLowerCase();

  if (lower.includes('timed out') || lower.includes('timeout')) {
    return 'The AI service took too long to respond. Please try again in a moment.';
  }
  if (lower.includes('network error') || lower.includes('fetch failed') || lower.includes('econnrefused')) {
    return 'We could not reach the AI service right now. Please try again shortly.';
  }
  if (lower.includes('api error') || lower.includes('not configured') || lower.includes('is missing')) {
    return 'The AI service is temporarily unavailable. Please try again shortly.';
  }

  return 'Something went wrong while generating your AI response. Please try again.';
}
 ...
 */
export function buildSuccessResponse({ message, data = null, user, options } = {}) {
 ...
 * @returns {StandardAIResponse}
 */
export function buildErrorResponse({ message, errors, user, options } = {}) {
  const { plan, usageRemaining } = extractUsageContext(user, options);
