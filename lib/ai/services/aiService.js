 ...
 * aiService.js
 * ---------------------------------------------------------------------------
 * SINGLE ENTRY POINT for every future AI feature (Resume Analyzer, Career
 * Coach, Cover Letter, Mock Interview, Job Match Score, etc.).
 *
 ...
 *
 * Usage from a future feature module (NOT built in this task):
 *
 *   const { runAIFeature } = require('../../lib/ai/services/aiService');
 *
 ...
 *     options: {
 *       supportedFeatures: ['resume-analyzer', 'career-coach', ...], // feature registry
 *       maxLength: 12000,
...
const { validateAIRequest } = require('../validators/validateRequest');
const { sanitize } = require('../validators/sanitizeInput');
const { resolveProvider, invokeProvider } = require('../utils/providerResolver');
 ...
 */
async function runAIFeature({ feature, user, prompt, options = {} } = {}) {
