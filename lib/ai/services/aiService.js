...
import { buildSuccessResponse, buildErrorResponse, toFriendlyErrorMessage } from './responseBuilder.js';
...
export async function runAIFeature({ feature, user, prompt, options = {} } = {}) {
  ...
  try {
    ...
    // Full detail (provider name, HTTP status, stack, etc.) stays in server
    // logs only — clients only ever see a short, friendly message below.
    logger.error('[aiService] AI request failed', { feature, error: err && err.message });
    return buildErrorResponse({
      message: 'AI request failed.',
      errors: [toFriendlyErrorMessage(err)],
      user,
      options,
    });
  }
