...
/**
 * TEMPORARY testing flag: keep every AI feature unlimited/free for every
 * user regardless of plan, since payments are not implemented yet.
 *
 * Usage is still recorded (AIUsage documents are still created/incremented
 * normally below) so the numbers are ready to enforce the moment this flag
 * is flipped back to `false` — this only disables the *blocking* behavior,
 * not the tracking.
 *
 * Flip this to `false` (or wire it to an env var) once billing/payments
 * ship and real plan limits should start being enforced again.
 */
export const TEMP_FREE_ACCESS = true;
